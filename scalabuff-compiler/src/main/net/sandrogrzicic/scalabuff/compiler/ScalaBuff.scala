...
object ScalaBuff {
  ...
  def processImportSymbols(tree: List[Node])(implicit settings: Settings = defaultSettings): Map[String, ImportedSymbol] = {
    def dig(name: String, isPublic: Boolean): List[(String, ImportedSymbol)] = {
      ...
      } ++ (if (!isPublic) List() else tree.collect {
        case ImportStatement(name, true) => dig(name.stripQuotes, true)
      }).flatten
    }
    tree.collect {
      case ImportStatement(name, isPublic) => dig(name.stripQuotes, isPublic)
    }.flatten.toMap
  }
